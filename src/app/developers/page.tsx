  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        body: `{"status": "hot", "tag": "interested", "score_label": "hot"}`,
        response: `{"updated": true, "lead": {"id": "...", "status": "hot"}}`,
      },
      {
        method: 'GET', path: '/api/v1/contacts/exist?phone=91XXXXXXXXXX',
        desc: 'Check if a contact exists by phone — scope: leads:read',
        body: null,
        response: `{"exists": true, "lead_id": "...", "name": "Rahul Sharma", "status": "new"}`,
      },
    ],
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        body: `{"to": "+91XXXXXXXXXX", "message": "Hi! Your order is ready."}`,
        response: `{"sent": true, "to": "+91XXXXXXXXXX", "message_id": "wamid..."}`,
      },
      {
        method: 'POST', path: '/api/v1/whatsapp/send-media',
        desc: 'Send image/video/document/audio by URL — scope: messages:send',
        body: `{"to": "+91XXXXXXXXXX", "media_type": "image", "media_url": "https://example.com/offer.jpg", "caption": "Festive offer!"}`,
        response: `{"sent": true, "media_type": "image", "wa_message_id": "wamid..."}`,
      },
      {
        method: 'POST', path: '/api/v1/whatsapp/send-bulk',
        desc: 'Send template to up to 100 numbers at once — scope: messages:send',
        body: `{"recipients": ["+91XXXXXXXXXX", "+91YYYYYYYYYY"], "template_name": "diwali_offer", "language_code": "en_US"}`,
        response: `{"total": 2, "sent": 2, "failed": 0, "results": [{"phone": "...", "status": "sent"}]}`,
      },
      {
        method: 'GET', path: '/api/v1/messages/{id}/status',
        desc: 'Delivery status: sent/delivered/read/failed — scope: messages:read',
        body: null,
        response: `{"id": "...", "delivery_status": "read", "sent_at": "...", "read_at": "..."}`,
      },
      {
    ...
    items: [
      {
        ...
        body: null,
        response: `[{"name": "Diwali 2026", "status": "completed", "sent": 450, "total": 500}]`,
      },
      {
        method: 'GET', path: '/api/v1/campaigns/{name}',
        desc: 'Campaign detail with per-recipient breakdown — scope: campaigns:read',
        body: null,
        response: `{"name": "Diwali 2026", "total": 500, "sent": 450, "recipients": [{"recipient": "+91...", "status": "sent"}]}`,
      },
      {
        method: 'POST', path: '/api/v1/campaigns/send',
        desc: 'Broadcast template to hot/warm/cold/all leads — scope: campaigns:write',
        body: `{"name": "june_promo", "template_name": "diwali_offer", "recipient_filter": "hot", "language_code": "en_US"}`,
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        desc: 'Create Razorpay payment link (amount in INR) — scope: payments:write',
        body: `{"customer_phone": "+91XXXXXXXXXX", "amount_inr": 499, "description": "Coaching fee - June batch", "lead_id": "lead_abc123"}`,
