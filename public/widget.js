(function () {
  ...
  function init(options) {
    ...
    wrapper.innerHTML = [
      ...
      ".brainai-bubble::after{content:'';position:absolute;bottom:-9px;left:50%;transform:translateX(-50%);border-left:9px solid transparent;border-right:9px solid transparent;border-top:6px solid #34b1c5;}",
      ".brainai-bubble.intro{background:rgba(8,10,20,.92);animation:brainai-pop 0.5s cubic-bezier(.175,.885,.32,1.275) forwards;color:#e2e8f0;border:1px solid rgba(0,242,254,.25);font-size:14px;font-weight:600;padding:12px 18px;box-shadow:0 16px 48px rgba(0,0,0,.5),0 0 24px rgba(0,242,254,.1);line-height:1.4;backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);}",
      ...
      /* Panel */
      ".brainai-panel{display:none;width:min(380px,calc(100vw - 32px));height:min(580px,calc(100vh - 112px));max-height:calc(100dvh - 112px);background:#fff;border-radius:20px;box-shadow:0 32px 80px rgba(17,24,39,.22),0 0 0 1px rgba(0,0,0,.06);overflow:hidden;overscroll-behavior:none;transform-origin:bottom right}",
    ...
    messages.addEventListener("wheel", function (e) {
      ...
      e.stopPropagation();
      if (messages.scrollHeight <= messages.clientHeight || (atTop && e.deltaY < 0) || (atBottom && e.deltaY > 0)) {
        e.preventDefault();
      }
    }, { passive: false });
    ...
    messages.addEventListener("touchmove", function (e) {
      ...
      var atBottom = messages.scrollTop + messages.clientHeight >= messages.scrollHeight - 1;
      e.stopPropagation();
      if (messages.scrollHeight <= messages.clientHeight || (atTop && dy < 0) || (atBottom && dy > 0)) {
        e.preventDefault();
      }
      _touchStartY = e.touches[0].clientY;
    }, { passive: false });
